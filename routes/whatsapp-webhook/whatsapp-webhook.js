		...
		if (
			...
			switch (messageType) {
				case "text":
					...
					if (
						message.text.body &&
						triggerPhase.includes(message.text.body.toLowerCase())
					) {
						...
						switch (user.userStat) {
							...
							case "FULL-NAME": {
								...
								if (nameRegex.test(textMessage)) {
									...
									sendRegistrationMessage(
										recipientNumber,
										"Patient's Birth-date of your Child/Patient. 📅 \nPlease enter it in the format DD/MM/YYYY (e.g., 01/01/2000)"
									);
									return res.sendStatus(200);
								} else {
									sendRegistrationMessage(
										recipientNumber,
										"Enter Proper Patient's Name ❌"
									);
									return res.sendStatus(200);
								}
							}
							...
							case "ON-SPECIFIC-DAY":
								function validateDateWithinNext7Days(dateString) {
									...
									return (
										inputDate.isSameOrAfter(currentDate, "day") &&
										inputDate.isSameOrBefore(next7Days, "day")
									);
								}
				...
				case "interactive":
					...
					if (
						...
						user.userStat === "DEPARTMENT-SELECTION"
					) {
						await db.WhatsappUser.update(
							{
						...
						if (listOfDoctor.length > 10) {
							...
							sendRegistrationMessage(
								recipientNumber,
								"AT THIS TIME NO DOCTOR AVAILABLE FOR THIS DEPARTMENT"
							);
							const listOfDepartment = await findDoctorDepartmentList();
						...
						interactiveType === "list_reply" &&
						user.userStat === "DOCTOR-SELECTION"
					) {
						await db.WhatsappUser.update(
							{
						...
						user.userStat === "DATE-SELECTION" &&
						reply.id === "todayButton"
					) {
						...
						interactiveType === "list_reply" &&
						user.userStat === "DAY-PART-SELECTION"
					) {
						console.log(listReply);
						if (
							listReply.id == "morning" ||
							listReply.id == "afternoon" ||
							listReply.id == "evening" ||
							listReply.id == "night" ||
							listReply.id == "midnight"
						) {
						...
						if (reply.id === "AGREE") {
							...
							sendRegistrationMessage(
								recipientNumber,
								"Apologies to inconvenience but we cannot proceed further until you agreed with terms & condition."
							);
						}
					}
